...
class Net(tf.layers.Model):
    def __init__(self):
        ...
        self.preBlock = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3D(24, kernel_size = 3, padding = "same"),
        ...
        self.maxpool3 = tf.keras.layers.MaxPooling3D(pool_size=2, strides=2)
        self.maxpool4 = tf.keras.layers.MaxPooling3D(pool_size=2, strides=2)
        ...
        self.path1 = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3DTranspose(filters=64, kernel_size = 2, stride = 2),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU()])
    ...
    def forward(self, x, coord):
        ...
        comb3 = self.back3(tf.concat((rev3, out3), 1))#96+96
