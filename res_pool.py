...
class Net(tf.keras.Model):
    def __init__(self):
        ...
        self.preBlock = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3D(16, kernel_size = 3, padding = "same"),
        ...
        for i in range(len(num_blocks)):
            ...
            for j in range(num_blocks[i]):
                if j == 0:
                    if i ==0:
                        blocks.append(tf.keras.layers.MaxPooling3D(pool_size=(1,2,2)))
                        blocks.append(PostRes(n_in[i], n_out[i]))
                    else:
                        blocks.append(tf.keras.layers.MaxPolling3D(pool_size=2))
        ...
        self.path2 = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3DTranspose(32, kernel_size = 2, stride = 2),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU()])

        self.path3 = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3DTranspose(32, kernel_size = 2, stride = 2),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU(),
            tf.keras.layers.Conv3DTranspose(32, kernel_size = 2, stride = 2),
        ...
        self.combine = tf.keras.layers.Sequential([
            tf.keras.layers.Conv3D(128, kernel_size = 1),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.ReLU()])

        self.drop = tf.keras.layers.Dropout(rate = 0.5)
        self.output = tf.keras.layers.Conv3D(5 * len(config['anchors']), kernel_size = 1)
    ...
    def forward(self, x):
        ...
        out = tf.concat((out2, out3, out4), 1)
